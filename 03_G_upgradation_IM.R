...
meta_data.TMO.HFX = meta_data[which(
  meta_data$Labcode == 'TMO' 
...
hfx.px.index = which(meta_data.TMO.HFX$UPerson == px)
hfx.px.fot = get_fot_df_by_expnames(fot_data, meta_data.TMO.HFX$Exp_code[hfx.px.index])
...
hfx.px.col_gps = apply(hfx.px.fot[,-1], 2, function(x, y){
  y[which(x>0)]
}, y = as.vector(unlist(hfx.px.fot[,1])))


e480.px.id = as.vector(unlist(lapply(e480.px.col_gps, length)))
hfx.px.id = as.vector(unlist(lapply(hfx.px.col_gps, length)))


barplot_df = data.frame(
  ID =  c(
    e480.px.id, nrow(e480.px.fot), 
    hfx.px.id, nrow(hfx.px.fot)
  ),
  Exptype = rep(c('R1', 'R2', 'R3', 'Union'), 2),
  Instrument = rep(c('Exploris480', 'QE-HFX'), each=4)
)
...
pdf(file="temp_figures/03/Ion_mobility/03_venn_QE-HFX.pdf")
...
#######################
#Exploris480 vs QE-HFX
# jco_col = pal_jco("default", alpha = 0.5)(10)[c(1,2)]
pdf(file="temp_figures/03/Ion_mobility/03_venn_e480_QE-HFX.pdf")
jco_col = pal_jco("default", alpha = 1)(10)[c(2,1)]
temp <- venn.diagram(
  x = list(
    Exploris480 = as.vector(e480.px.fot$GeneSymbol),
    'QE-HFX' = as.vector(hfx.px.fot$GeneSymbol)
  ),
  filename = NULL,
  main = 'Exploris480 vs QE-HFX',
  # main.just = c(0, 5),
  main.pos = c(0.5,1.05),
  main.cex = 0.5,
  output = TRUE ,
...
hfx.row_id = apply(hfx.px.fot[,-1], 1, function(x){length(which(x>0))})
...
hfx.row_mean = rowMeans(hfx.px.fot[,-1])
hfx.row_expr_label = get_expr_label(hfx.row_mean)
...
hfx.df = data.frame(
  GeneSymbol = hfx.px.fot$GeneSymbol,
  RowID = hfx.row_id,
  ExprLab = hfx.row_expr_label
)


table(e480.df$RowID, e480.df$ExprLab)
table(hfx.df$RowID, hfx.df$ExprLab)
...
if(T){
  ...
  # hfx.px.col_gps, hfx.df
  hfx.repro = get_px_reproducibility(hfx.px.col_gps, ref_df = NULL, ref_lab = NULL)
  hfx.repro.h = get_px_reproducibility(hfx.px.col_gps, ref_df = hfx.df, ref_lab = 'High')
  hfx.repro.m = get_px_reproducibility(hfx.px.col_gps, ref_df = hfx.df, ref_lab = 'Medium')
  hfx.repro.l = get_px_reproducibility(hfx.px.col_gps, ref_df = hfx.df, ref_lab = 'Low')
  df.hfx = data.frame(
    Intensity = rep(c('Low', 'Medium', 'High', 'Global'), each = 6),
    Reproducibity = c(hfx.repro.l, hfx.repro.m, hfx.repro.h, hfx.repro)
  )
  df.hfx$Intensity = factor(df.hfx$Intensity, levels = c('Low', 'Medium', 'High', 'Global'))
  ...
  # e480.px.fot
  if(T){
    hfx.px.fot.row_id = apply(hfx.px.fot[,-1], 1, function(x){length(which(x>0))})
    e480.px.fot.row_id = apply(e480.px.fot[,-1], 1, function(x){length(which(x>0))})
    
    px.fot = merge(
      hfx.px.fot[hfx.px.fot.row_id>=1,], 
...
timstof.h.gps = hfx.df$GeneSymbol[hfx.df$ExprLab=='High']
...
timstof.m.gps = hfx.df$GeneSymbol[hfx.df$ExprLab=='Medium']
...
timstof.l.gps = hfx.df$GeneSymbol[hfx.df$ExprLab=='Low']
length(intersect(e480.l.gps, timstof.l.gps))/length(union(e480.l.gps, timstof.l.gps))


# Sankey
hfx_e480.sankey_df <- generate_sankey_df( hfx.df, e480.df,c('QE-HFX', 'Exploris480'))
...
# 相关性
hfx.df$FOT = apply(hfx.px.fot[,-1], 1, mean)
e480.df$FOT = apply(e480.px.fot[,-1], 1, mean)



merge_df = merge(hfx.df, e480.df, by = 'GeneSymbol', all = T)
...
cor.g <- ggpairs(
  ...
  columnLabels = c('Exploris480-Global', 'QE-HFX-Global'),
  lower=list(continuous=my_fn)
)
...
get_ggplot_cor_density <- function(dat, label){
  ...
  p <- p + xlab("log10(FOT)/Exploris480") + ylab("log10(FOT)/QE-HFX") 
...
hfx.px.fot.common = hfx.px.fot[match(common_genes,hfx.px.fot$GeneSymbol),]
...
hfx.g.cv = get_cv_list(hfx.px.fot.common)
hfx.h.cv = get_cv_list(hfx.px.fot.common[merge_df$ExprLab.x=='High',])
hfx.m.cv = get_cv_list(hfx.px.fot.common[merge_df$ExprLab.x=='Medium',])
hfx.l.cv = get_cv_list(hfx.px.fot.common[merge_df$ExprLab.x=='Low',])
...
hfx_e480.g.cv = get_cv_list(
  data.frame(hfx.px.fot.common, e480.px.fot.common[,-1])
)
hfx_e480.h.cv = get_cv_list(
  data.frame(hfx.px.fot.common, e480.px.fot.common[,-1])[h_index,]
)
hfx_e480.m.cv = get_cv_list(
  data.frame(hfx.px.fot.common, e480.px.fot.common[,-1])[m_index,]
)
hfx_e480.l.cv = get_cv_list(
  data.frame(hfx.px.fot.common, e480.px.fot.common[,-1])[l_index,]
)

bp <- boxplot(
  hfx.l.cv, hfx.m.cv, hfx.h.cv, hfx.g.cv,
...
if(T){#8*6
  ...
  axis(
    1, at = c(1.75, 6.25), labels = c('QE-HFX', 'Exploris480')
  )
...
hfx.fot = get_fot_df_by_expnames(fot_data, meta_data.NPB.timstof$Exp_code)
...
intersect_genes = intersect(as.vector(e480.fot$GeneSymbol), as.vector(hfx.fot$GeneSymbol))
min_fc = 2
e480.fot.deps_list = get_deps(e480.fot[match(intersect_genes, e480.fot$GeneSymbol),], meta_data.NPB.e480$UPerson, min_fc=2)
hfx.fot.deps_list = get_deps(hfx.fot[match(intersect_genes, hfx.fot$GeneSymbol),], meta_data.NPB.timstof$UPerson, min_fc=2)

get_consensus_deps = function(df1, df2, min_fc = 2){
  df1 = e480.fot.deps_list[[1]]
  df2 = hfx.fot.deps_list[[1]]
...
if(T){
  # PCA
  hfx.fot = get_fot_df_by_expnames(fot_data, meta_data.TMO.HFX$Exp_code)
  e480.fot = get_fot_df_by_expnames(fot_data, meta_data.TMO.E480$Exp_code)
  
  # hfx.fot
  hfx.fot.row_mean = rowMeans(hfx.fot[,-1])
  hfx.fot.row_expr_label = get_expr_label(hfx.fot.row_mean)
  hfx.fot.df = data.frame(
    GeneSymbol = hfx.fot$GeneSymbol,
    RowID = apply(hfx.fot[,-1], 1, function(x){length(which(x>0))}),
    MeanFOT = hfx.fot.row_mean,
    ExprLab = hfx.fot.row_expr_label
  )
