...
barplot_df = data.frame(
  ...
  Instrument = rep(c('timsTOF', 'QTOF6600'), each=4)
)
barplot_df$Instrument = factor(barplot_df$Instrument, levels = c('QTOF6600', 'timsTOF'))
...
venn.diagram(
  ...
  filename = 'temp_figures/03/TOF/03_venn_timstof.png',
  main = 'timsTOF',
  # main.just = c(0, 5),
  main.pos = c(0.5,1.05),
  main.cex = 0.5,
...
venn.diagram(
  ...
  filename = 'temp_figures/03/TOF/03_venn_timsTOF_QTOF6600_per.png',
  main = 'QTOF6600 vs timsTOF',
  # main.just = c(0, 5),
  main.pos = c(0.5,1.05),
  main.cex = 0.5,
  output = TRUE ,
...
if(T){
  ...
  df$Instrument = rep(
    c('QTOF6600', 'timsTOF'), each = 24
  )
  df$Instrument = factor(df$Instrument, levels = c('QTOF6600', 'timsTOF'))
...
merge_df$FOT.x[is.na(merge_df$FOT.x)] = 0
merge_df$FOT.y[is.na(merge_df$FOT.y)] = 0
...
get_ggplot_cor_density <- function(dat, label, xlab, ylab){
  # dat = log10(dat)
  df <- data.frame(
    x = dat[,1],
    y = dat[,2]
  ...
  p <- p + xlab(paste("log10(FOT)/", xlab, sep = '')) + ylab(paste("log10(FOT)/", ylab, sep = '')) 
...
xlab = "QTOF6600"
ylab = "timsTOF"
p.g = get_ggplot_cor_density((merge_df[,c(4,7)]), 'Global', xlab, ylab)
p.h = get_ggplot_cor_density((merge_df.high[,c(4,7)]), 'High', xlab, ylab)
p.m = get_ggplot_cor_density((merge_df.medium[,c(4,7)]), 'Medium', xlab, ylab)
p.l = get_ggplot_cor_density((merge_df.low[,c(4,7)]), 'Low', xlab, ylab)
...
merge_df = na.omit(merge_df)
# Global
# common_genes = as.vector(merge_df$GeneSymbol[!is.na(merge_df$ExprLab.x)])
# h_index = which(merge_df$ExprLab.x=='High')
# m_index = which(merge_df$ExprLab.x=='Medium')
# l_index = which(merge_df$ExprLab.x=='Low')
# qtof6600.px.fot.common = qtof6600.px.fot[match(common_genes,qtof6600.px.fot$GeneSymbol),]
# 
# 
# common_genes = as.vector(merge_df$GeneSymbol[!is.na(merge_df$ExprLab.y)])
# h_index = which(merge_df$ExprLab.y=='High')
# m_index = which(merge_df$ExprLab.y=='Medium')
# l_index = which(merge_df$ExprLab.y=='Low')
...
qtof6600.h.cv = get_cv_list(qtof6600.px.fot.common[merge_df$ExprLab.x=='High',])
qtof6600.m.cv = get_cv_list(qtof6600.px.fot.common[merge_df$ExprLab.x=='Medium',])
qtof6600.l.cv = get_cv_list(qtof6600.px.fot.common[merge_df$ExprLab.x=='Low',])


timstof.g.cv = get_cv_list(timstof.px.fot.common)
timstof.h.cv = get_cv_list(timstof.px.fot.common[merge_df$ExprLab.y=='High',])
timstof.m.cv = get_cv_list(timstof.px.fot.common[merge_df$ExprLab.y=='Medium',])
timstof.l.cv = get_cv_list(timstof.px.fot.common[merge_df$ExprLab.y=='Low',])


qtof6600_timstof.g.cv = get_cv_list(
  data.frame(qtof6600.px.fot.common, timstof.px.fot.common[,-1])
)
qtof6600_timstof.h.cv = get_cv_list(
  data.frame(qtof6600.px.fot.common, timstof.px.fot.common[,-1])[h_index,]
)
qtof6600_timstof.m.cv = get_cv_list(
  data.frame(qtof6600.px.fot.common, timstof.px.fot.common[,-1])[m_index,]
)
qtof6600_timstof.l.cv = get_cv_list(
  data.frame(qtof6600.px.fot.common, timstof.px.fot.common[,-1])[l_index,]
)
...
if(T){#8*6
  ...
  boxplot(
    qtof6600_timstof.l.cv, qtof6600_timstof.m.cv, qtof6600_timstof.h.cv, qtof6600_timstof.g.cv,
  ...
  axis(
    1, at = c(1.75, 6.25, 10.75), labels = c('QTOF6600', 'timsTOF', 'QTOF6600&timsTOF')
  )
...
if(T){
  ...
  qtof6600.px.fot.cv_list = get_cv_list_set(qtof6600.px.fot, min_id = 3)
  timstof.px.fot.cv_list = get_cv_list_set(timstof.px.fot, min_id = 3)
  ...
  if(T){
    ...
    boxplot(
      qtof6600.px.fot.cv_list$l_cv, qtof6600.px.fot.cv_list$m_cv,
      qtof6600.px.fot.cv_list$h_cv, qtof6600.px.fot.cv_list$g_cv,
      at = c(0.25, 1.25, 2.25, 3.25), boxwex = 0.5,
      add = T, xaxt = 'n', yaxt = 'n', col = c('#33C860', '#81B0FF', '#F9918A', '#FFCC00')
    )
    
    boxplot(
      timstof.px.fot.cv_list$l_cv, timstof.px.fot.cv_list$m_cv,
      timstof.px.fot.cv_list$h_cv, timstof.px.fot.cv_list$g_cv,
      at = c(0.25, 1.25, 2.25, 3.25)+4.5, boxwex = 0.5,
      add = T, xaxt = 'n', yaxt = 'n', col = c('#33C860', '#81B0FF', '#F9918A', '#FFCC00')
    )
    ...
    axis(
      ...
      labels = c('QTOF6600', 'timsTOF', 'G3', 'G4', 'G5')[1:2]
    )
...
if(F){
  ...
  qtof6600_timsTOF.df = links
  qtof6600_timsTOF.df.table = get_probs_table(qtof6600_timsTOF.df)
  output_path = normalizePath(path = file.path(
    BASE_DIR, 'sankey','QTOF6600-timsTOF-sankey-probs.xlsx'
  ),mustWork = F)
  
  write.xlsx(
    qtof6600_timsTOF.df.table, output_path, sheetName = 'qtof6600_timsTOF'
  )
  ...
  # PCA
  qtof6600fot = get_fot_df_by_expnames(fot_data, meta_data.BRK.QTOF6600$Exp_code)
  timsTOFfot = get_fot_df_by_expnames(fot_data, meta_data.BRK.timsTOF$Exp_code)
  
  qtof6600fot.ref = get_fot_ref_df(qtof6600fot)
  timsTOFfot.ref = get_fot_ref_df(timsTOFfot)
  
  pdf(
    normalizePath(
      file.path(BASE_DIR, 'sankey', '20200825', 'PCA_qtof6600_timsTOF.pdf'), mustWork = F
    ),
    height = 8, width = 8
  )
  g1.snr_list = plot_single_pca(qtof6600fot, qtof6600fot.ref, main_lab = 'QTOF6600', p_flags, p_colors)
  g2.snr_list = plot_single_pca(timsTOFfot, timsTOFfot.ref, main_lab = 'timsPro', p_flags, p_colors)
